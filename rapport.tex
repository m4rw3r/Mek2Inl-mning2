...
\section{Problem 1}
	...
	Låt $\omega$ vara $xyz$-systemets rotationsvektor, och därmed även kroppens rotationsvektor.
...
\section{Problem 2}
	...
	Vi betraktar fallet då $\mathbf{M} = \mathbf{0}$ och $\boldsymbol{\Omega}$ konstant relativt $\hat{x}\hat{y}\hat{z}$-systemet:
...
\label{IoejIz}
	...
	\begin{equation*}
		\Omega_z = \frac{\nu_z I_z}{I_o-I_z}
		\hspace{12pt}
		\Omega = \frac{\Omega_z}{\cos(b/R)} = \frac{\nu_z I_z}{\cos(b/R)(I_o-I_z)}
	\end{equation*}
...
\section{Problem 3}
	...
	\subsection{Beräkningar}
		...
		I det fall då vi har vinkelhastighet i något led fås ingen
		vinkelaccelleration i vår modell. Detta visade sig vara svårt att åstadkomma i praktiken.
	...
	\end{center}
		...
		Om däremot $y$ är den dominerande rotationsaxeln och $\omega_x \ne 0 \land \omega_z \ne 0$
		så resulterar det i att rotationen kommer att förflytta sig mellan $y$ till $x$ och $z$
		för att sedan komma tillbaka till $y$. Dvs. initial rotation kring $y$ är instabil.
		...
		Även extremt små avvikelser från en ren $\hat{y}$-rotation (i storleksordningen $10^{-10}$) påverkar rotationen markant,
